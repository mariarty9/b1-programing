...
store = UserStore("users.db")                  # Now using database file!!
...
def search_users(q: str = None):
    ...
    users = store.load()
    results = [user for user in users if q.lower() in user['name'].lower()]          # Filters users whose names contain the search term
    return results
